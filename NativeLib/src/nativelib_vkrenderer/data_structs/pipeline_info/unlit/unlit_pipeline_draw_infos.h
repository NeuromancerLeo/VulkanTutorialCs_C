...
typedef struct UnlitMaterialDrawInfo {
    ...
    /// @brief 该材质实例的 Material 描述符集中，描述符 MaterialUniformBuffer 的大小
    ///（为创建 Buffer 时申请的大小！内部用于计算当前飞行帧对应的偏移）
    size_t                  uniformBufferDescBufferSize;
...
typedef struct UnlitPipelineBindingDrawInfo {
    ...
    /// @brief 绘制所需绑定的 DrawItem 描述符集中，描述符 DrawItemUniformBuffer 的大小
    ///（为创建 Buffer 时申请的大小！内部用于计算当前飞行帧对应的偏移）
    size_t                  drawItemUniformBufferDescBufferSize;

    /// @brief 绘制所需绑定的 DrawItem 描述符集.
    VkDescriptorSet         drawItemDescSet;
...
typedef struct UnlitPipelineDrawInfo {
    ...
    /// @brief 绑定的 Camera 描述符集中，描述符 CameraUniformBuffer 的大小
    ///（为创建 Buffer 时申请的大小！内部用于计算当前飞行帧对应的偏移）
    size_t                              cameraUniformBufferDescBufferSize;

    /// @brief 绑定 Camera 描述符集时针对描述符 CameraUniformBuffer 要应用的动态偏移量
    uint64_t                            cameraUniformBufferDescDynamicOffset;

    /// @brief 要绑定的 Camera 描述符集
